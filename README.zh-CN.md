...
SuperXP v1 是一个最小 Codex 和 Kimi Code 插件预览，核心包含两个 skills：
...
然后重启 Codex 或开启新的对话，让已经安装的 skills 被加载。

### Kimi Code

正式版安装使用 GitHub 仓库默认分支，也就是 `main`：

```text
/plugins install https://github.com/howiehu/superxp
```

开发版请安装 `dev` 分支的本地 checkout：
...
然后在 Kimi Code 中安装本地插件路径：

```text
/plugins install /path/to/superxp
```

然后重启 Kimi Code 或开启新的会话。
...
- `/using-superxp`
- Kimi Code 中的 `/skill:using-superxp`
- `$using-superxp`
- 以 `xp!` 开始用户消息，大小写均可，例如 `xp! help me slice this change`。

唯一允许的隐式匹配，是由 `using-superxp` 处理的消息开头显式关键字 `xp!`。内部 `xp-loop` skill 禁用隐式调用。在 Kimi Code 中，`/skill:using-superxp` 是可靠的显式入口；`xp!` 依赖 Kimi Code 原生的 skill 选择能力。
